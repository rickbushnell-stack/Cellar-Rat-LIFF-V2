...
const SommelierChat: React.FC<SommelierChatProps> = ({ cellar }) => {
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: `UI Critical Error: ${err.message}`, 
        timestamp: Date.now() 
      }]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col h-[600px] bg-[#1a1616] border border-[#2d2424] rounded-xl overflow-hidden shadow-2xl">
      <div className="p-4 bg-[#231d1d] border-b border-[#3a3030] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-tighter text-gray-500 font-bold">Live Diagnostic Active</p>
          </div>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-[url('https://www.transparenttextures.com/patterns/dark-leather.png')]">
        {messages.length === 0 && (
          <div className="text-center py-10">
            <p className="text-[#c8a97e] font-serif text-lg mb-2">Welcome to the inner sanctum.</p>
            <p className="text-xs text-gray-500 max-w-xs mx-auto italic mb-6">Test the connection below or ask any wine question.</p>
            <div className="flex flex-wrap justify-center gap-2">
              <button 
                onClick={() => handleSend("What is a good way to describe red wines?")}
                className="text-xs px-4 py-2 bg-[#4a0e0e] text-[#c8a97e] border border-[#c8a97e]/30 rounded-full hover:bg-[#601212] transition-colors"
              >
                Test Connection
              </button>
            </div>
          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] p-4 rounded-lg text-sm leading-relaxed ${
              msg.role === 'user' 
                ...
                : msg.text.startsWith('API Error') 
                  ? 'bg-red-950/50 border border-red-500 text-red-200'
                  : 'bg-[#231d1d] border border-[#3a3030] text-gray-200 rounded-bl-none shadow-xl'
            }`}>
              {msg.text}
            </div>
          </div>
